...
func (ws *wasmServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "/scroll_worker.js":
		w.Header().Set("Content-Type", "application/javascript")
		w.Header().Set("Content-Length", strconv.Itoa(len(scrollWorkerJS)))
		if _, err := w.Write(scrollWorkerJS); err != nil {
			ws.logger.Println("unable to write scroll_worker.js.")
		}
	default:
